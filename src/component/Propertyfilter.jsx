...
export default function PropertyFilters() {
  ...
  const propertyFilterData = [
    {title:'Hide already seen',data:'input',name:'already seen'},
  ...
  const [openMobileIndex, setMobileOpenIndex] = useState(null);
          ...
          >
            {propertyFilterData.map((item, index) => (
              <li
                key={index}
                className={`${
                  item.data === "input" ? "hidden" : "py-3 flex sm:py-4 cursor-pointer"
                }`}
  ...
  <div className="flex-1 p-6">
    {propertyFilterData.map((item, index) => (
      <div
        key={index}
        className={`${index === openMobileIndex ? "block" : "hidden"}`}
      >
